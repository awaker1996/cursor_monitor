...
- 系统托盘悬停展示概览用量摘要（随快照刷新更新）；右键菜单为查看流水、应用设置、重置悬浮、退出。
- 自动刷新与可配置刷新间隔。
...
## 4. 术语说明
- auto：Cursor Models 池（原 First-party models / Auto + Composer，对应 Dashboard 自有模型用量）。
- api：Other Models 池（原 API，非 Cursor 自有模型调用相关 token 指标）。
...
- **概览视图**至少包括：
  ...
  - 四张纵向 metric 卡片：今日 Cursor Models、今日 Other Models、周期 Cursor Models、周期 Other Models
  - `auto.remaining`、`auto.limit`、`api.remaining`、`api.limit`（若接口提供，用于余量计算）
- **用量视图**（有 Included Usage 数据时）：
  - 按模型聚合的 Included Usage 列表（Cursor Models / Other Models 分区）
...
### FR-06 托盘与生命周期
- 提供托盘菜单：查看流水、应用设置、重置悬浮、退出（不包含立即刷新与暂停/恢复）。
- 悬停托盘图标时展示多行概览用量摘要（总消耗、今日/周期 Cursor Models 与 Other Models 分项、来源与更新时间；无快照时显示「暂无数据」）。
...
### FR-10 用量流水窗口
- 托盘「查看流水」打开独立窗口，不依赖 poller 快照自动刷新。
...
- 托盘悬停：多行概览用量摘要（见 FR-06）；右键为查看流水 / 应用设置 / 重置悬浮 / 退出。
...
- 托盘悬停显示多行概览用量，随刷新更新；右键含查看流水、应用设置、重置悬浮、退出。
