...
- 目标：提供一个轻量、常驻、可配置自动刷新的悬浮球，展示 `auto` 与 `api` 用量概览、周期明细及余量。
- 目标用户：个人开发者（账号所有者本人）。

## 3. 范围定义
### 3.1 范围内
- Windows 桌面常驻悬浮球（可拖拽、置顶、贴边收起、托盘管理）。
- 显示 `auto` 与 `api` 两类 token 余量及 Dashboard 用量指标（概览 / Included 明细分段切换）。
- 系统托盘悬停展示概览用量摘要（随快照刷新更新）。
- 自动刷新与可配置刷新间隔。
...
- 展开态默认展示概览；有 Included Usage 数据时可切换至明细视图。
- 支持贴边缘自动收起为 peek tab，点击或拖出恢复。

### FR-02 明细展示
- **概览视图**至少包括：
  - 总消耗百分比、周期 token 汇总、状态 pill、来源、最近成功刷新时间
  - 四张 metric 卡片：今日 API、今日 First-party models、周期 API、周期 First-party models
  - `auto.remaining`、`auto.limit`、`api.remaining`、`api.limit`（若接口提供，用于余量计算）
- **明细视图**（有 Included Usage 数据时）：
  - 按模型聚合的 Included Usage 表格
  - `billingCycleStart` / `billingCycleEnd` 展示于 Included Usage 区块
  - 表格区域独立滚动；概览视图无纵向滚动条
...
- 贴边收起态：peek tab + 迷你圆环 + 健康点。
- 展开态 · 概览：总消耗 dashboard、4 张 metric 卡片、状态/来源/时间、刷新/设置/收起；面板进出场动画（上浮 + 淡入 + 微缩放）。
- 展开态 · 明细：Included Usage 表格；有数据时底部分「概览 / 明细」分段切换；长模型名溢出时慢速单行滚动。
- 未贴边窗口固定 360×520；贴边 peek 为 300×448；展开/收起与视图切换不改变未贴边窗体尺寸。
- 设置页：自动刷新、刷新间隔（30–3600 秒）、贴边开关、自定义图标、Cookie 输入、连接测试（结构化详情面板）、清除凭据。
- 托盘悬停：多行概览用量摘要（见 FR-06）。
