...
- 目标：提供一个轻量、常驻、可配置自动刷新的悬浮球，展示 `auto` 与 `api` 两类余量。
- 目标用户：个人开发者（账号所有者本人）。

## 3. 范围定义
### 3.1 范围内
- Windows 桌面常驻悬浮球（可拖拽、置顶、托盘管理）。
- 显示 `auto` 与 `api` 两类 token 余量。
- 自动刷新与可配置刷新间隔。
- 数据来源优先级：官方接口优先，失败后回退 Cookie 接口。
- 设置页支持手动录入 Cookie、刷新配置、连通性测试。
...
- 默认开启自动刷新。
- 默认刷新间隔为 30 秒。
- 用户可在设置页修改刷新间隔（秒），建议允许范围 10-3600。
...
- 回退后仍需后台探测首选 Provider 恢复情况，并可自动切回。
- 切换过程应在 UI 上有可见来源标识与状态说明。

### FR-05 Cookie 配置与测试
- 设置页提供手动粘贴 Cookie 的输入区域。
...
- 凭据需存储在系统安全存储（Windows Credential Manager）。
- 日志需对敏感信息脱敏后输出。
...
## 8. 交互与界面需求
- 悬浮球折叠态：总览 + 健康点。
- 展开态：auto/api 明细、来源、最后更新时间、错误提示。
- 设置页：Cookie 输入、自动刷新开关、刷新间隔输入、连接测试、清除凭据。
- 错误态提示应简洁明确，优先可操作建议（如“检查 Cookie 是否过期”）。

## 9. 验收标准
- 启动后 5 秒内出现悬浮球并可拖拽/置顶。
- 能稳定展示 `auto` 与 `api` 两类数据，且标识数据来源。
...
## 11. 版本计划
- v1：单账号、实时显示、自动刷新、双 Provider 回退。
...
### 12.3 CookieProvider 映射规则（示例）
- 输入示例（字段名可能变化，仅作实现参考）：
```json
{
  "usage": {
    "auto": { "left": 118000, "cap": 200000, "reset_time": "2026-07-31T00:00:00Z" },
    "api": { "left": 76000, "cap": 100000, "reset_time": "2026-07-31T00:00:00Z" }
  }
}
```
- 映射规则：
  - `usage.auto.left -> TokenSnapshot.auto.remaining`
  - `usage.auto.cap -> TokenSnapshot.auto.limit`
  - `usage.auto.reset_time -> TokenSnapshot.auto.resetAt`
  - `usage.api.left -> TokenSnapshot.api.remaining`
  - `usage.api.cap -> TokenSnapshot.api.limit`
  - `usage.api.reset_time -> TokenSnapshot.api.resetAt`
  - `source = "cookie"`
  - `rawVersion` 按接口版本记录，如 `cookie:v1`
